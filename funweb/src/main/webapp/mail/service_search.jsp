...
serviceDAO boardDAO = new serviceDAO();
...
ArrayList<serviceDTO> boardList = boardDAO.selectSearchBoardList(pageNum, listLimit, search,searchType);
...
// 				}
				...
				for(serviceDTO board : boardList) {
				%>
			...
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='service_write.jsp'">
			</div>
			<div id="table_search">
				
				<form action="service_search.jsp" method="get">
		...
		<!-- 페이징 처리 -->
			...
			<div id="page_control">
				...
				<%if(pageNum > 1) { // 이전페이지가 존재할 경우 %>
					<a href="service_search.jsp?page=<%=pageNum - 1%>&search=<%=search%>&searchType=<%=searchType%>">Prev</a>
				...
				<%for(int i = startPage; i <= endPage; i++) { %>
					...
					<%if(pageNum == i) { %>
						&nbsp;&nbsp;<%=i %>&nbsp;&nbsp;
					<%} else { %>
						<a href="service_search.jsp?page=<%=i%>&search=<%=search%>&searchType=<%=searchType%>"><%=i %></a>
					<%} %>
				<%} %>
				<!-- 
				...
				<%if(pageNum < maxPage) { // 다음페이지가 존재할 경우 %>
					<a href="service_search.jsp?page=<%=pageNum + 1%>&search=<%=search%>&searchType=<%=searchType%>">Next</a>
				<%} else { // 이전페이지가 존재하지 않을 경우 %>
					&nbsp;Next
				<%} %>
			</div>
