...
String PageNum = request.getParameter("page");
	...
	serviceDTO board = dao.selectBoard(num);  //글(상세내용) 조회 메서드
		...
		<article>
			...
			<div id="table_search">
				...
				<input type="button" value="글수정" class="btn" 
						onclick="location.href='service_update.jsp?num=<%=board.getNum() %>&page=<%=PageNum %>'">
				<!-- 글 수정 클릭 시 notice_delete.jsp 페이지로 이동(글번호, 페이지번호 전달) -->
				<input type="button" value="글삭제" class="btn" 
						onclick="location.href='service_delete.jsp?num=<%=board.getNum() %>&page=<%=PageNum %>'">
				<!-- 글목록 버튼 클릭 시 notice.jsp 페이지로 이동(페이지번호 전달) --> 
				<input type="button" value="글목록" class="btn" 
						onclick="location.href='service.jsp?page=<%=PageNum%>'">
			</div>
